...
import net.pufferlab.primitivelife.items.ItemPrimitiveBucket;
...
public class BucketHandler {
    ...
    public void swingItemPacket(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            ...
            PrimitiveLife.networkWrapper.sendTo(new PacketSwingArm(player), playerMP);
        }
    }
    ...
    public void playerInteractEventHandler(PlayerInteractEvent event) {
        if (!event.world.isRemote && event.entityPlayer.getCurrentEquippedItem() != null) {
            if (event.entityPlayer.getCurrentEquippedItem()
                .getItem() instanceof ItemPrimitiveBucket bucket) {
