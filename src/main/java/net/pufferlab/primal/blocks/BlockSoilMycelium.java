...
public class BlockSoilMycelium extends BlockMetaMycelium {

    public SoilType[] stoneTypes;

    public BlockSoilMycelium(SoilType[] materials, String type) {
        ...
        if (!worldIn.isRemote) {
            if (worldIn.getBlockLightValue(x, y + 1, z) < 4 && worldIn.getBlockLightOpacity(x, y + 1, z) > 2) {
                ...
                for (int l = 0; l < 4; ++l) {
                    ...
                    if (BlockUtils.isDirtBlock(blockCurrent) && worldIn.getBlockLightValue(i1, j1 + 1, k1) >= 4
