...
public class WDHarvestHandler implements HarvestHandler {

    public static final ResourceLocation resourceLocation = new ResourceLocation(Primal.MODID + ":wdharvesthandler");

    private static final Map<EffectiveTool, ItemStack> testTools = new HashMap<>();
    ...
    public static final EffectiveTool TOOL_SWORD = new EffectiveTool(
        "sword",
        Arrays.asList(new ItemStack(Registry.flint_knife)));
        ...
        if (phase == HarvestabilityTestPhase.EFFECTIVE_TOOL_NAME) {
            if (!info.getEffectiveTool()
                ...
                for (Map.Entry<EffectiveTool, ItemStack> testTool : testTools.entrySet()) {
                    if (info.getEffectiveTool()
                        .isSameTool(testTool.getKey())) {
                        info.setEffectiveTool(testTool.getKey());
                    }
                }
                if (info.getEffectiveTool()
                    .isSameTool(TOOL_SWORD)) {
