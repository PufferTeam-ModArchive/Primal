...
public class Perlin2D implements Noise2D {
    ...
    @Override
    public Perlin2D octaves(int octaves) {
    ...
    @Override
    public Perlin2D scaled(float min, float max) {
        return scaled(-1, 1, min, max);
    }

    @Override
    public Perlin2D scaled(float oldMin, float oldMax, float min, float max) {
